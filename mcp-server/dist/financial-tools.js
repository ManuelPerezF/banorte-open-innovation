...
export async function analyzeCompanyKPIs(params) {
    try {
...
export async function predictTrends(params) {
    try {
...
function generateAreaRecommendations(area, currentPercent, reductionPercent) {
...
export async function calculateScenarios(params) {
    try {
        ...
        const bestScenario = scenarioResults.reduce((best, current) => current.resultados.margen.nuevo > best.resultados.margen.nuevo ? current : best);
        return {
            success: true,
...
async function generateCompanyReport(companyId, reportType, period) {
    const { data: kpis, error } = await supabase
        .from('v_company_kpis')
        .select('*')
        .eq('empresa_id', companyId)
...
export async function getPersonalFinancialSummary(userId) {
    try {
...
async function generatePersonalReport(userId, reportType, period) {
...
export async function getCompanyFinancialContext(companyId) {
    try {
        const { data: kpis, error } = await supabase
            .from('v_company_kpis')
            .select('*')
